...
//        logout(result)
        ...
        codeChallenge = arguments["codeChallenge"] as String
        zaloInstance.authenticateZaloWithAuthenType(activity, LoginVia.APP_OR_WEB, codeChallenge, extInfo, listener2)
        result.success("ok")
//          var listener: OAuthCompleteListener = object : OAuthCompleteListener() {
//
...
//          }
    ...
    private val listener2 : OAuthCompleteListener = object : OAuthCompleteListener(){
        override fun onAuthenError(errorResponse: ErrorResponse?) {
            super.onAuthenError(errorResponse)
        }

        override fun onGetOAuthComplete(res: OauthResponse) {
            super.onGetOAuthComplete(res)
            val oauthCode = res.oauthCode
            zaloInstance.getAccessTokenByOAuthCode(activity, oauthCode, codeVerifier, withZOGraphCallBackWithoutResult())
        }
    }
    ...
    @Throws(Exception::class)
    private fun withZOGraphCallBackWithoutResult(): ZaloOpenAPICallback {
        return ZaloOpenAPICallback { response: JSONObject? ->
            try {
                if (response == null) {
                    val error: MutableMap<String, Any?> = HashMap()
                    ...
                    map["error"] = error
//                    result.success(map)
                    this.channel.invokeMethod("result_login_zalo", map);
                } else {
                    val data: Map<String, Any?> = AppHelper.jsonToMap(response)
                    val errorCode = data["error"] as Int
                    val isSuccess = errorCode == 0
